    ...
    <header className="relative pt-20 pb-16 px-6 sm:px-8 md:px-12 lg:px-16 overflow-visible">
      <div className="relative max-w-7xl mx-auto grid lg:grid-cols-[1.15fr_1fr] gap-8 lg:gap-12 items-center">
        ...
        <div className="text-center lg:text-left lg:max-w-[560px] w-full mx-auto lg:mx-0">
          ...
          {/* Action buttons */}
          <div className="animate-fade-up delay-300 flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3.5">
            <Link
              to="/signup"
            ...
            <Link
              to="/pricing"
              className="w-full sm:w-auto bg-card border border-border px-7 py-3 rounded-lg text-sm font-semibold hover:border-brand/40 hover:bg-card/85 transition-all duration-200 btn-press text-foreground"
            >
              See pricing
            </Link>
          </div>
              ...
              <div key={step.title} className="flex items-center md:contents">
                <div className="flex flex-col items-center text-center gap-2 flex-1 md:px-1">
                  <div className="relative size-9 rounded-full bg-[var(--landing-blue-tint)] border border-[var(--landing-blue-border)] grid place-items-center">
                    ...
                    <step.icon className="size-4 text-[var(--landing-blue-bright)]" />
                  </div>
                  <p className="font-bold text-foreground text-[12px]">{step.title}</p>
                  <p className="text-[10px] text-muted-foreground leading-snug max-w-[120px]">{step.desc}</p>
                </div>
                {i < workflowSteps.length - 1 && (
                  <ArrowRight className="hidden md:block size-4 text-muted-foreground/40 mx-0.5 shrink-0" />
                )}
              </div>
            ))}
          </div>
...
function PanelShell({ activeNav, breadcrumb, children }: { activeNav: string; breadcrumb: string; children: ReactNode }) {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden border border-border/70 shadow-elevated gradient-border h-full">
      ...
      <div className="flex h-[360px] sm:h-[390px] md:h-[420px] bg-card">
        <aside className="w-40 shrink-0 border-r border-border p-3 hidden lg:flex flex-col gap-4 bg-[oklch(0.14_0.024_265)]">
          ...
          <nav className="space-y-0.5">
            {navItems.map((label) => (
              <div
                key={label}
                className={`px-2.5 py-1.5 rounded-md text-[12px] font-medium transition-all border-l-2 ${
                  label === activeNav
          ...
          <div className="mt-auto bg-background/60 p-2.5 rounded-lg border border-border/60 space-y-1.5">
            <div className="flex items-center gap-1">
              <Sparkles className="size-3.5 text-brand" />
              <p className="text-[9px] font-bold text-brand uppercase tracking-wider">AI Insight</p>
            </div>
        ...
        <main className="flex-1 flex flex-col min-w-0">
          <div className="px-4 py-2 border-b border-border/60 flex items-center justify-between bg-background/20 shrink-0">
...
function ResearchPanel() {
  return (
    <PanelShell activeNav="Research" breadcrumb="Research  ›  Helix Commerce">
      <div className="h-full overflow-y-auto p-3.5 sm:p-4 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <div className="size-9 rounded-lg bg-brand/10 border border-brand/25 grid place-items-center shrink-0">
              <Building2 className="size-4 text-brand" />
            </div>
            <div>
              <p className="font-bold text-foreground text-xs">Helix Commerce</p>
              <p className="text-[10px] text-muted-foreground">B2B E-commerce Platform</p>
              <p className="text-[10px] text-muted-foreground flex items-center gap-1 mt-0.5">
                <MapPin className="size-3.5" /> New York, United States
              </p>
            </div>
          </div>
          <div className="text-right shrink-0 bg-background/50 border border-border/60 rounded-lg px-2 py-1">
            <p className="text-base font-bold text-foreground leading-none">92</p>
            <p className="text-[8px] text-muted-foreground uppercase tracking-wider mt-0.5">Score</p>
          </div>
        </div>

        <div className="flex items-center gap-3 border-b border-border/50 text-[10px] font-semibold text-muted-foreground pb-1.5">
          <span className="text-brand border-b-2 border-brand pb-1.5 -mb-1.5">Overview</span>
          <span>Contacts</span>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <div>
            <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider mb-1.5">Key Information</p>
          ...
          <div>
            <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider mb-1.5">Contacts</p>
            <div className="space-y-1.5 text-[11px]">
              {[
                { icon: Mail, v: "info@helixcommerce.com", sub: "General" },
                { icon: Mail, v: "sarah@helixcommerce.com", sub: "Sales Head" },
                { icon: Phone, v: "+1 (212) 555-0187", sub: "Main Number" },
              ].map((c) => (
                <div key={c.v} className="flex items-start gap-1.5">
                  <c.icon className="size-4 text-muted-foreground shrink-0" />
                  <div className="min-w-0">
                    <p className="text-foreground font-medium truncate">{c.v}</p>
                    <p className="text-muted-foreground text-[9px]">{c.sub}</p>
                  </div>
                  <CheckCircle2 className="size-3.5 text-success ml-auto shrink-0" />
                </div>
              ))}
              <p className="text-brand text-[10px] font-semibold pt-0.5">View all contacts →</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="h-full overflow-y-auto p-3.5 sm:p-4 space-y-3">
        <div className="grid grid-cols-4 gap-1.5">
          {[
            ...
            <div key={stage} className={`${bg} border border-border/40 rounded-lg p-1.5 text-center`}>
              <p className={`text-base font-bold ${color}`}>{count}</p>
              <p className="text-[8px] text-muted-foreground font-medium mt-0.5">{stage}</p>
            </div>
          ))}
        </div>

        <div className="bg-background/40 border border-border/60 rounded-lg overflow-hidden">
          <table className="w-full text-left text-xs">
            <thead className="bg-card/60 border-b border-border/60">
              <tr>
                {["Company", "Score", "Stage"].map((h) => (
                  <th key={h} className="p-2 font-semibold text-muted-foreground text-[9px] uppercase tracking-wider">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {prospects.map((row, i) => (
                <tr key={row.name} className={i < prospects.length - 1 ? "border-b border-border/40" : ""}>
                  <td className="p-2 text-foreground font-medium text-[11px]">{row.name}</td>
                  <td className="p-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[9px] text-muted-foreground font-mono">{row.score}</span>
                    </div>
                  </td>
                  <td className="p-2">
                    <span className={`px-1 py-0.5 text-[8px] rounded border font-semibold uppercase tracking-wider ${stageColor[row.stage] ?? ""}`}>
                      {row.stage}
                    </span>
                  </td>
                </tr>
        ...
        <div className="rounded-lg border border-brand/20 p-2.5 bg-background/40">
          <div className="flex items-center gap-1.5 mb-1.5">
            <Sparkles className="size-4 text-brand" />
            <p className="text-[9px] font-bold text-brand uppercase tracking-wider">AI Insight</p>
          </div>
          <p className="text-[11px] text-foreground leading-relaxed min-h-[30px]">
            {aiText}
            <span className="inline-block w-0.5 h-2.5 bg-brand ml-0.5 animate-pulse align-middle" />
          </p>
        </div>
      </div>
    </PanelShell>
...
function RelationshipsPanel() {
  const contacts = [
    { name: "Sarah Whitfield", role: "Head of Sales · Helix", tag: "Warm", tagColor: "bg-success/10 text-success border-success/20" },
  ...
  return (
    <PanelShell activeNav="Research" breadcrumb="Relationships  ›  All Contacts">
      <div className="h-full overflow-y-auto p-3.5 space-y-2">
        {contacts.map((c) => (
          <div key={c.name} className="flex items-center gap-2 bg-background/40 border border-border/50 rounded-lg p-2 hover:border-brand/30 transition-colors">
            <div className="size-7 rounded-full bg-brand/10 border border-brand/25 grid place-items-center text-[10px] font-bold text-brand shrink-0">
              {c.name.split(" ").map((n) => n[0]).join("")}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[11.5px] font-semibold text-foreground truncate">{c.name}</p>
              <p className="text-[10px] text-muted-foreground truncate">{c.role}</p>
            </div>
            <span className={`shrink-0 px-1.5 py-0.5 rounded border text-[8.5px] font-semibold uppercase tracking-wider ${c.tagColor}`}>
              {c.tag}
            </span>
          </div>
        ))}
        <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground pt-0.5">
          <Tag className="size-4" /> 1,842 relationships tracked across your team
        </div>
      </div>
    </PanelShell>
  );
  ...
  return (
    <PanelShell activeNav="Insights" breadcrumb="Insights  ›  Weekly Intelligence">
      <div className="h-full overflow-y-auto p-3.5 space-y-2.5">
        <div className="rounded-lg border border-brand/25 p-2.5 bg-background/40">
          <div className="flex items-center gap-1.5 mb-1">
            <Sparkles className="size-4 text-brand" />
            <p className="text-[9px] font-bold text-brand uppercase tracking-wider">AI Recommendation</p>
          </div>
        ...
        {[
          ...
          { icon: Globe2, label: "Northwind Studio relaunched website", time: "1d ago" },
        ].map((s) => (
          <div key={s.label} className="flex items-start gap-2 bg-background/30 border border-border/40 rounded-lg p-2">
            <s.icon className="size-4 text-brand shrink-0 mt-0.5" />
            <div>
              <p className="text-[11px] text-foreground leading-snug">{s.label}</p>
        ...
        <div className="grid grid-cols-3 gap-1.5 pt-0.5">
          {[
            ...
            <div key={s.l} className="text-center bg-background/40 border border-border/40 rounded-lg py-1.5">
...
function RemindersPanel() {
  ...
  return (
    <PanelShell activeNav="Focus" breadcrumb="Focus  ›  Today's Follow-ups">
      <div className="h-full overflow-y-auto p-3.5 space-y-2">
        {reminders.map((r) => (
          <div key={r.label} className="flex items-center gap-2.5 bg-background/40 border border-border/50 rounded-lg p-2">
            <div className="size-3.5 rounded border border-border shrink-0" />
            <div className="min-w-0 flex-1">
              <p className="text-[11.5px] font-medium text-foreground truncate">{r.label}</p>
              <p className="text-[10px] text-muted-foreground flex items-center gap-1 mt-0.5">
                <Clock className="size-3.5" /> {r.due}
              </p>
            </div>
            <span className={`shrink-0 px-1.5 py-0.5 rounded border text-[8.5px] font-semibold uppercase tracking-wider ${r.color}`}>
              {r.priority}
            </span>
          </div>
        ))}
        <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground pt-0.5">
          <Star className="size-4 text-brand" /> 5 follow-ups due in 48 hours
        </div>
      </div>
    </PanelShell>
  );
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-center gap-1.5 mb-8 animate-fade-up">
          {showcaseTabs.map((t) => (
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-semibold border transition-all duration-200 btn-press ${
                active === t.id
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-10 items-center">
          ...
          <div key={tab.id} className="animate-fade-up">
            ...
            <h2 className="mt-3 text-[clamp(1.5rem,2.8vw,2rem)] font-bold tracking-tight leading-[1.12]">
              {tab.headline}
              <br />
              {tab.headlineAccent}
            </h2>
            <p className="mt-3 text-muted-foreground leading-relaxed text-[0.85rem]">{tab.desc}</p>
            ...
            <Link
              to="/signup"
              className="group mt-5 inline-flex items-center gap-2 bg-[var(--landing-blue)] text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-[var(--landing-blue-hover)] transition-all duration-200 btn-press shadow-[0_10px_20px_-10px_var(--landing-blue)]"
            >
              {tab.cta}
    ...
    <section id="solutions" className="relative py-16 px-6">
      <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-12 items-center">
        <div className="animate-fade-up">
          ...
          <p className="mt-4 text-muted-foreground leading-relaxed text-[0.85rem]">
          ...
          <ul className="mt-5 space-y-2">
            {bullets.map((t, i) => (
              <li
                key={t}
                className="flex items-start gap-2.5 text-[13px] text-foreground animate-slide-right"
        ...
        <div ref={ref} className="animate-fade-up delay-200">
          <div className="relative bg-card border border-border/60 rounded-2xl p-6 shadow-elevated card-hover gradient-border">
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: "Active deals", value: n1, suffix: "", sub: "across stages" },
                { label: "Follow-ups due", value: n2, suffix: "", sub: "in 48 hours" },
                ...
                <div key={label} className="rounded-xl bg-background/60 border border-border/50 p-3 hover:border-brand/30 transition-colors duration-200">
            ...
            {/* AI card */}
            <div className="mt-4 rounded-xl border border-brand/20 p-3 bg-background/40">
              <div className="flex items-center gap-1.5 mb-1.5">
                <Sparkles className="size-4 text-brand" />
                <p className="text-[9px] font-bold text-brand uppercase tracking-wider">AI Recommendation</p>
              </div>
              <p className="text-[13px] text-foreground leading-relaxed">
                Luminal AI hasn't had an update in 6 days and their funding round closes Friday.
                Consider moving this opportunity forward.
              </p>
              <p className="mt-1.5 text-[9px] text-muted-foreground">MAST Intelligence · Suggested action</p>
            </div>
          </div>
        </div>
      </div>
...
export function PlanCard({ name, price, forWho, outcome, features, cta, popular }: {
  ...
  return (
    ...
    >
      {popular && (
        <>
          <div className="absolute top-0 right-0 bg-brand text-brand-foreground px-3 py-1 text-[9px] font-bold uppercase tracking-wider rounded-bl-lg">
            Most Popular
          </div>
      ...
      {/* Price */}
      <div className="text-[1.8rem] font-bold text-foreground leading-none">
        {price}
        <span className="text-xs text-muted-foreground font-normal">/mo</span>
      </div>

      {/* Who it's for */}
      {forWho && <p className="text-[10px] text-muted-foreground mt-1 font-medium">{forWho}</p>}
      ...
      <Link
        to="/signup"
        className={`w-full py-2 rounded-lg font-bold transition-all duration-200 btn-press text-center text-xs ${
          popular
    ...
    <section className="relative py-16 px-6">
      <div className="max-w-4xl mx-auto relative rounded-2xl overflow-hidden border border-border/60 p-10 text-center gradient-border">
        ...
        <div className="relative">
          <p className="animate-fade-up text-[10px] font-bold text-[var(--landing-blue-bright)] uppercase tracking-[0.2em] mb-4">Get started today</p>
          <h2 className="text-[clamp(1.6rem,4vw,2.2rem)] font-bold tracking-tight leading-tight animate-fade-up delay-100">
            Your sales operation,<br />finally in one place.
          </h2>
          <p className="mt-3 text-muted-foreground max-w-md mx-auto text-[0.85rem] leading-relaxed animate-fade-up delay-200">
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-2.5 justify-center animate-fade-up delay-300">
            <Link
              to="/signup"
              className="group relative bg-[var(--landing-blue)] text-white px-6 py-2.5 rounded-lg text-sm font-semibold shadow-[0_10px_20px_-10px_var(--landing-blue)] hover:bg-[var(--landing-blue-hover)] transition-all duration-200 btn-press overflow-hidden"
            >
            ...
            <Link
              to="/pricing"
              className="bg-background border border-border/80 px-6 py-2.5 rounded-lg text-sm font-semibold hover:border-brand/40 hover:bg-brand/5 transition-all duration-200 btn-press"
            >
              View pricing
            </Link>
          </div>
