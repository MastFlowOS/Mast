-- MAST Phase 2B hotfix follow-up: resolve PL/pgSQL output-variable ambiguity.
-- claim_discovery_street RETURNS TABLE with a street_id output parameter;
-- #variable_conflict use_column makes ON CONFLICT's street_id resolve to the
-- target table column while preserving the exact expression conflict target.
--
-- Migration 029 created an expression unique index:
